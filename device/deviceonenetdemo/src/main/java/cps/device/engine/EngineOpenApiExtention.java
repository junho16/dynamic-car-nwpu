package cps.device.engine;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.onenet.studio.acc.sdk.OpenApi;
import com.onenet.studio.acc.sdk.interfaces.OpenApiCallback;
import cps.device.engine.dto.SetEngineStateStructDTO;
import cps.device.engine.dto.StartTheAbnormalStructDTO;
import cps.device.engine.dto.TemperatureWarningStructDTO;
 ...
 */
public class EngineOpenApiExtention {
    /**
     * 物模型上报格式公共字段-id
     */
    ...
    public EngineOpenApiExtention(OpenApi openApi) {
     ...
     * 单个属性功能点上报
     *
     *  @param powerStatus 标识符为powerStatus的属性功能点的值
     *  @param timeout 超时时间，单位为毫秒
     */
    public int powerStatusPropertyUpload(Integer powerStatus, long timeout) throws Exception {
        return this.propertyUpload(timeout, powerStatus, null);
    }

    /**
     * 单个属性功能点上报
     *
     *  @param temperature 标识符为temperature的属性功能点的值
     *  @param timeout 超时时间，单位为毫秒
     */
    public int temperaturePropertyUpload(Double temperature, long timeout) throws Exception {
        return this.propertyUpload(timeout, null, temperature);
    }
     ...
     *  @param temperature 标识符为temperature的属性功能点的值
     */
    public int propertyUpload(long timeout, Integer powerStatus, Double temperature) throws
            Exception {
        ...
        if (!Objects.isNull(powerStatus)) {
                Map<String, Object> val1 = new HashMap<>();
                val1.put("value", powerStatus);
                val1.put("time", now);
                params.put("powerStatus", val1);
        }
        if (!Objects.isNull(temperature)) {
                Map<String, Object> val2 = new HashMap<>();
                val2.put("value", temperature);
                val2.put("time", now);
                params.put("temperature", val2);
        }
     ...
     *
     *  @param timeout 调用超时时间,单位为毫秒
     *  @param startTheAbnormal 事件输出标识符为startTheAbnormal的参数
     *  @param temperatureWarning 事件输出标识符为temperatureWarning的参数
     */
    public int eventUpload(long timeout, StartTheAbnormalStructDTO startTheAbnormal,
            TemperatureWarningStructDTO temperatureWarning) throws Exception {
        Map<String, Object> oneJson = new HashMap<>();
        long now = System.currentTimeMillis();
        String id = String.valueOf(now);
        oneJson.put(ONEJSON_ID_KEY, id);
        oneJson.put(ONEJSON_VERSION_KEY, ONEJSON_VERSION_VAL);
        Map<String, Object> params = new HashMap<>();
        if (!Objects.isNull(temperatureWarning)) {
                Map<String, Object> val1 = new HashMap<>();
                val1.put("value", JSON.toJSON(temperatureWarning));
                val1.put("time", now);
                params.put("temperatureWarning", val1);
        }
        if (!Objects.isNull(startTheAbnormal)) {
                Map<String, Object> val2 = new HashMap<>();
                val2.put("value", JSON.toJSON(startTheAbnormal));
                val2.put("time", now);
                params.put("startTheAbnormal", val2);
        }
        oneJson.put("params", params);
        String oneJsonStr = JSON.toJSONString(oneJson);
    ...
    /**
     * 标识符为setEngineState的设备服务调用订阅
     *
     *  @param callback 设备接收消息回调方法，由用户自定义实现
     */
    public void setEngineStateServiceInvoke(OpenApiCallback callback) throws Exception {
        this.openApi.serviceInvokeSubscribe("setEngineState", callback);
    }
     ...
     *  @param code 返回状态码，200为成功
     *  @param msg 返回消息
     *  @param setEngineState 标识符为setEngineState的服务输出参数
     */
    public void setEngineStateServiceInvokeReply(String messageId, Integer code, String msg,
            SetEngineStateStructDTO setEngineState) throws Exception {
        Map map = new HashMap();
        map.put("id", messageId);
        map.put("code", code);
        map.put("msg", msg);
        map.put("data", JSON.toJSON(setEngineState));
        String oneJsonStr = JSON.toJSONString(map);
        this.openApi.serviceInvokePublish("setEngineState", oneJsonStr);
    }
}
