 ...
 */
public class OpenApiExtentionForSdkTest {
    /**
     * 物模型上报格式公共字段-id
     */
    ...
    public OpenApiExtentionForSdkTest(OpenApi openApi) {
     ...
     * 单个属性功能点上报
     *
     *  @param CurrentTemperature 标识符为CurrentTemperature的属性功能点的值
     *  @param timeout 超时时间，单位为毫秒
     */
    public int CurrentTemperaturePropertyUpload(Double CurrentTemperature, long timeout) throws
            Exception {
        return this.propertyUpload(timeout, CurrentTemperature, null);
    }

    /**
     * 单个属性功能点上报
     *
     *  @param CurrentHumidity 标识符为CurrentHumidity的属性功能点的值
     *  @param timeout 超时时间，单位为毫秒
     */
    public int CurrentHumidityPropertyUpload(Double CurrentHumidity, long timeout) throws
            Exception {
        return this.propertyUpload(timeout, null, CurrentHumidity);
    }
     ...
     *  @param CurrentHumidity 标识符为CurrentHumidity的属性功能点的值
     */
    public int propertyUpload(long timeout, Double CurrentTemperature, Double CurrentHumidity)
            throws Exception {
        ...
        if (!Objects.isNull(CurrentHumidity)) {
                Map<String, Object> val1 = new HashMap<>();
                val1.put("value", CurrentHumidity);
                val1.put("time", now);
                params.put("CurrentHumidity", val1);
        }
        if (!Objects.isNull(CurrentTemperature)) {
                Map<String, Object> val2 = new HashMap<>();
                val2.put("value", CurrentTemperature);
                val2.put("time", now);
                params.put("CurrentTemperature", val2);
        }
