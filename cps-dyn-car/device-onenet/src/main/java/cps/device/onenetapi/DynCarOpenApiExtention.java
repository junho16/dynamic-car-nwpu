 ...
 */
public class DynCarOpenApiExtention {
    /**
     * 物模型上报格式公共字段-id
     */
    ...
    public DynCarOpenApiExtention(OpenApi openApi) {
     ...
     *
     *  @param timeout 调用超时时间,单位为毫秒
     *  @param DrivingSuggest 事件输出标识符为DrivingSuggest的参数
     */
    public int eventUpload(long timeout, DrivingSuggestStructDTO DrivingSuggest) throws Exception {
        Map<String, Object> oneJson = new HashMap<>();
        long now = System.currentTimeMillis();
        String id = String.valueOf(now);
        oneJson.put(ONEJSON_ID_KEY, id);
